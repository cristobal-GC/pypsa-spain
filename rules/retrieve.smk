...
if (EU_NUTS2021_DATASET := dataset_version("eu_nuts2021"))["source"] in [
    ...
    rule retrieve_eu_nuts_2021:
        ...
        output:
            ...
            shapes_all=f"{EU_NUTS2021_DATASET['folder']}/ref-nuts-2021-01m.geojson/NUTS_RG_01M_2021_4326.geojson",   ##### required as input in build_electricity_demand_base.py for the PyPSA-Spain methodology
        run:
            copy2(input["shapes"], output["zip_file"])
            unpack_archive(output["zip_file"], Path(output.shapes_level_3).parent)
